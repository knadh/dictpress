...
res AS (
    ...
    SELECT COUNT(*) OVER () AS total, entries.*, TS_RANK(tokens, (SELECT query FROM q), 1) AS rank FROM entries WHERE
        ($4 = '' OR lang=$4)
...
UNION ALL
    ...
    SELECT COUNT(*) OVER () AS total, entries.*, LENGTH(content) AS rank FROM entries WHERE
        ($4 = '' OR lang=$4)
        AND (COALESCE(CARDINALITY($5::TEXT[]), 0) = 0 OR types && $5)
        AND (COALESCE(CARDINALITY($6::TEXT[]), 0) = 0 OR tags && $6)
        AND (LOWER(SUBSTRING(content, 0, 50))=LOWER(SUBSTRING($1, 0, 50)) OR tokens @@ TO_TSQUERY('simple', $2))
        AND NOT EXISTS (SELECT * FROM res)
        ORDER BY rank DESC
        OFFSET $7 LIMIT $8
