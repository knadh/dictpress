...
```shell
./dictpress --db=data.db --site=./site
```

The site will be served on the port set in the configuration file. eg: `http://localhost:9000`. To customize the site, edit the template files in the `site` directory. Templates use the Django/Jinjna2-style [tera](https://keats.github.io/tera/docs/) templating language.
