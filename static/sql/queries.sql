...
-- name: get-all-words
-- Fetch all words for a given language (enabled entries with relations only).
-- Returns first content element lowercased, sorted ASC.
-- $1: lang
SELECT DISTINCT LOWER(JSON_EXTRACT(content, '$[0]')) AS word
FROM entries
WHERE lang = $1 AND status = 'enabled'
